...
public:
	...
	void AddForce( Force* force )
	{
		...
		LinearDynamicsState3D* lds = new LinearDynamicsState3D();
...
private:
	...
	void AssignEphanovParticleStates( float baseDistance, float nonPlanarDepth, const Vector3f& velocity = Vector3f::ZERO ) //Note: 0,0 == top-left, so +x is right, +y is down.
	{
		...
		LinearDynamicsState3D* lds = new LinearDynamicsState3D(); //Need its forces to stay valid over cloth lifetime, EphanovParticle will handle cleanup.
		...
		for ( int r = 0; r < m_numRows; r++ )
		{
			for ( int c = 0; c < m_numCols; c++ )
			{
				...
				currentEphanovParticle->SetEphanovParticleState( new LinearDynamicsState3D( startPosition, velocity ) ); //EphanovParticle will handle state cleanup.
