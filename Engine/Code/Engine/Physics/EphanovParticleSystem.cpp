	...
	, m_secondsPassedSinceLastEmit( 0.f )
{
	...
	m_EphanovParticleToEmit.SetEphanovParticleState( new LinearDynamicsState3D( emitterPosition, Vector3f::ZERO ) ); //So we can add forces to it prior to emission if requested.
...
void EphanovParticleSystem::EmitEphanovParticles( float deltaSeconds )
{
	if ( m_secondsPassedSinceLastEmit >= m_secondsBetweenEmits )
	{
		...
		for ( unsigned int iterationNum = 0; iterationNum < m_EphanovParticlesEmittedAtOnce; iterationNum++ )
		{
			...
			newEphanovParticle->SetEphanovParticleState( new LinearDynamicsState3D( newEphanovParticlePosition, muzzleVelocity ) );
