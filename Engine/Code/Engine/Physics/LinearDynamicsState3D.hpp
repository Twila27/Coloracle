...
//-----------------------------------------------------------------------------
class LinearDynamicsState3D //Future variant or subclass of an entity's rigid-body PhysicsComponent. Distinct from AngularDynamics (see MP3 notes).
{
public:

	LinearDynamicsState3D( const Vector3f& position = Vector3f::ZERO, const Vector3f& velocity = Vector3f::ZERO )
		: m_position( position )
		, m_velocity( velocity )
	{
	}
	~LinearDynamicsState3D();
...
private:
	...
	LinearDynamicsState3D dStateForMass( float mass ) const; //Solves accel, for use in Step() integrators.
