...
//--------------------------------------------------------------------------------------------------------------
static void ToggleCursor( Command& )
{
	if ( g_theInput == nullptr )
		return;
	g_theInput->ToggleCursor();
	ShowCursor( g_theInput->IsCursorVisible() );
}
static void FreeCursor( Command& )
{
	if ( g_theInput == nullptr )
		return;
...
TheInput::TheInput()
	: m_mouseWheelDelta( 0 )
{
	g_theConsole->RegisterCommand( "ToggleMouse", GLOBAL::ToggleCursor );
	g_theConsole->RegisterCommand( "FreeMouse", GLOBAL::FreeCursor );
...
//--------------------------------------------------------------------------------------------------------------
bool TheInput::IsButtonDown( ControllerButtons buttonID, Controllers controllerID /*= Controllers::CONTROLLER_ONE*/ ) const
{
