...
static const bool DID_NOT_REACH_END = false; //More semantic-only things.
static const bool DID_INDEED_REACH_END = true;


//-----------------------------------------------------------------------------
#define KB * 1024
typedef uint32_t nuonce_t; //"Number used only once."
...
//-----------------------------------------------------------------------------
//Utility
extern void LogAndShowPrintf( const char* messageFormat, ... ); //Invokes Logger + Console printf.
extern void LogAndShowPrintfWithTag( const char* tag, const char* messageFormat, ... );
...
//-----------------------------------------------------------------------------
//State Machines
typedef const char* StateNameStr;
...
static const bool SHOULD_NOT_UNSUB = false;
static const bool SHOULD_INDEED_UNSUB = true;


//--------------------------------------------------------------------------------------------------------------
//Networking
typedef uint8_t NetConnectionIndex;
#define MAX_GUID_LENGTH	(32)
enum NetCoreMessageType : uint8_t //Limited to 256 total message types, 0-255.
{
	NO_MSG_TYPE_ID = 0,
	NETMSG_PING,
	NETMSG_PONG,
	...
	NETMSG_JOIN_DENY,
	NETMSG_LEAVE,
	MAX_CORE_NETMSG_TYPES //Assigned to the first game-side message type.
};