...
{
	unsigned int vertexBufferID = ( mesh ? mesh->GetVertexBufferID() : NULL ); //Unbind if nullptr was sent.
	unsigned int indexBufferID = ( mesh ? mesh->GetIndexBufferID() : NULL );

	glBindVertexArray( m_vaoID );
	glBindBuffer( GL_ARRAY_BUFFER, vertexBufferID );
	glBindBuffer( GL_ELEMENT_ARRAY_BUFFER, indexBufferID );

	glBindVertexArray( NULL ); //Must come prior to below two lines, but why?
...
void MeshRenderer::SetMaterial( Material* material, bool overwriteMemberMaterial )
{
	if ( m_mesh == nullptr )
	{
		ERROR_RECOVERABLE( "Cannot SetMaterial without a mesh in place!" );
		return;
	}
