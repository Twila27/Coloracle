...
{
	TextRenderer text = TextRenderer( TEXT_PROPORTIONAL_2D, false, //Do not Enable/Register to the static array, this is a fire-and-forget usage.
							 lowerLeftOriginPos,
							 inputText,
							 scale,
							 font,
	...
	TextRenderer::RenderText( &text ); //No need to enable-disable since this is just a 1-off.
	return;
}
...
void TheRenderer::Update( float deltaSeconds, const Camera3D* activeCam )
{
	...
	TextRenderer::UpdateAllScreenText();
}
...
//--------------------------------------------------------------------------------------------------------------
void TheRenderer::RenderAllScreenText()
{
...
//#else
	...
	Vector2f topRight = Vector2f( (float)GetScreenWidth(), (float)GetScreenHeight() );

	g_theRenderer->SetOrtho( bottomLeft, topRight );
	TextRenderer::SetupView2D( bottomLeft, topRight );
 //#endif
}
