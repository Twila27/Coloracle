...
#ifdef LOGGER_OUTPUT_TO_CONSOLE
	
	if ( g_theConsole != nullptr )
	{
		FIXME( "ThreadSafeVector TheConsole::m_storedText won't afford looping, but without it Logger changes contents midway into loop!" );
		//g_theConsole->Printf( msg->buffer );
	}
#endif
}
...
void Logger::LoggerThreadEntry( void* )
{
	...
	static unsigned int fileNum = 0; //For netgames, first instance has debug0.log, next debug1.log, etc.
	...
	while ( Logger::ShouldRun() )
	{
		...
		if ( m_logFile == nullptr ) 
		{
			do
			{
				openedFile = TryCreateFile( &m_logFile, Stringf( "%s%u%s", LOG_FILE_NAME, fileNum++, LOG_FILE_EXTENSION ).c_str(), "ab" );;
				//Can't use Error library if this fails, since it depends on Logger it would yield an infinite loop.
			}
			while ( !openedFile );
		}
		...
		const std::chrono::milliseconds THREAD_SLEEP_TIME( 2000 );
		Thread::ThreadSleep( THREAD_SLEEP_TIME ); //Lets other threads run on the CPU instead of this one if we have no messages to process.
	}
