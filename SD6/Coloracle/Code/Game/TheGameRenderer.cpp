...
//Game-side Use of SD5 Rendering System Elements
...
//--------------------------------------------------------------------------------------------------------------
bool TheGame::ClientRender2D( EngineEvent* eventContext )
{
	g_theRenderer->PreRenderStep();
	g_theRenderer->SetupView2D( g_theGame->GetActiveCamera2D() );

	//Disseminating events manually rather than via state machine to decouple from engine and state logic for netgame needs.
	StateNameStr currentStateName = m_gameStateMachine.GetCurrentState()->GetName();
	if ( strcmp( currentStateName, "TheGameState_Title" ) == 0 )
		TheEventSystem::Instance()->TriggerEvent( "TheGame_RenderTitle" );
	else if ( strcmp( currentStateName, "TheGameState_Lobby" ) == 0 )
		TheEventSystem::Instance()->TriggerEvent( "TheGame_RenderLobby" );
	else if ( strcmp( currentStateName, "TheGameState_MidMatch" ) == 0 )
		TheEventSystem::Instance()->TriggerEvent( "TheGame_RenderMidMatch" );
	else if ( strcmp( currentStateName, "TheGameState_PostMatch" ) == 0 )
		TheEventSystem::Instance()->TriggerEvent( "TheGame_RenderPostMatch" );

	float deltaSeconds = dynamic_cast<EngineEventUpdate*>( eventContext )->deltaSeconds;
	TheGameDebug::Instance()->HandleDebug( deltaSeconds );

	TextRenderer::RenderAllScreenText();
...
//--------------------------------------------------------------------------------------------------------------
bool TheGame::RenderDebug3D( EngineEvent* )
{
	TODO( "Need to figure out how best to decouple from TheEngine and delete." );
...
//-----------------------------------------------------------------------------
bool TheGame::RenderDebug2D( EngineEvent* /*eventContext*/ )
{
	TODO( "Need to figure out how best to decouple from TheEngine and delete." );
